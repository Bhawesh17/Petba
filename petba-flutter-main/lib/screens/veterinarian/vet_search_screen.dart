...
class VetSearchPage extends StatefulWidget {
  static const id = 'vet_search_screen';
  @override
...
class _VetSearchPageState extends State<VetSearchPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      Provider.of<VetSearchProvider>(context, listen: false).fetchVetData();
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final vets = Provider.of<VetSearchProvider>(context);
    return Scaffold(
      body: SafeArea(
        child: Column(
          children: [
            AppBarWidget(title: 'Veterinarian', width: 160),
            vets.isLoading
                ? LoadingVets(windowHeight: windowHeight)
                : Container(
                    height: windowHeight - 55,
                    child: SingleChildScrollView(
                      child: Column(
                        children: [
                          for (int i = 0; i < vets.vets.length; i++)
                            (VetContainer(
                              result: vets.vets[i],
                            ))
                        ],
...
class VetContainer extends StatelessWidget {
  double windowWidth;
  VetContainer({this.result});
  final Places result;

  @override
  Widget build(BuildContext context) {
    windowWidth = MediaQuery.of(context).size.width;
    // return Text(address.adminArea);
    return Container(
...
class LoadingVets extends StatelessWidget {
  LoadingVets({this.windowHeight});
